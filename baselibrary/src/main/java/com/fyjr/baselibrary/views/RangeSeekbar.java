...
public class RangeSeekbar extends View {
    ...
    private void applyConfig(Context context, AttributeSet attrs) {
        ...
        maxNum = a.getInteger(R.styleable.RangeSeekbar_maxNum, 1000);
        ...
        if (mTextArray != null && mTextArray.length > 0) {
            ...
            mTextArrLength = mTextArray.length - 1;
            multiple = maxNum / mTextArrLength;
        }
...
//                            * mPartLength - fromX, 0, mDuration);
    ...
    private void handleTouchMove(MotionEvent event) {
        ...
        if (mLeftHited && mLeftPointerID != -1) {
            ...
            if (mLeftCursorRect.right + deltaX >= mRightCursorRect.left) {
                ...
                if (mRightHited || mRightCursorIndex >= mTextArray.length - 1
                    ...
                    if (mRightCursorIndex <= maxMarkIndex) {
                        mRightCursorNextIndex = (int) (mRightCursorIndex + 1);
                        if (mRightCursorNextIndex > mTextArrLength) {
                            mRightCursorNextIndex = mTextArrLength;
                        }
                        ...
                        if (!mRightScroller.computeScrollOffset()) {
                            ...
                            mTextArray[mTextArrLength] = roundNum((int) (mRightCursorNextIndex * multiple));
            ...
            mTextArray[0] = roundNum((int) (mLeftCursorIndex * multiple));
        ...
        if (mRightHited && mRightPointerID != -1) {
            ...
            if (mRightCursorRect.left + deltaX < mLeftCursorRect.right) {
                if (mLeftHited || mLeftCursorIndex == 0
                    ...
                    if (mLeftCursorIndex >= 0) {
                        ...
                        if (mLeftCursorNextIndex < 0) {
                            mLeftCursorNextIndex = 0;
                        }
                        ...
                        if (!mLeftScroller.computeScrollOffset()) {
                            ...
                            mTextArray[0] = roundNum((int) (mLeftCursorNextIndex * multiple));
            ...
            mTextArray[mTextArrLength] = roundNum((int) (mRightCursorIndex * multiple));

            invalidate();
        }
    }

    public String roundNum(int num) {
        num = (int) Math.round(num / 10.0);
        return "" + (num * 10);
    }
